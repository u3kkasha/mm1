...
Event(Event::Event_kind chosen_event, int chosen_trigger_time, int chosen_target_entity_number)
        : Event(chosen_event, chosen_trigger_time)
    {
        if (chosen_target_entity_number >= 0)
            target_entity_number = chosen_target_entity_number;
	else throw std::domain_error("Received a negative target_entity_number");
    }
    void handle()
    {
        ...
        : event_type(chosen_event)
    {
        ...
        else throw std::domain_error("Received a negative trigger_time");
    }
