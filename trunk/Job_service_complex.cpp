...
namespace Singleton {
    ...
    {
        //checks if system_number is the last index, because index starts from 0 we need to subtract one from size()
    ...
    {
        Job unprocessed_job;
        job_processing_systems[0].arrival(unprocessed_job);
    }
     void Job_service_complex:: departure(int target_system_number)
    {
        ...
        if (is_terminal_job_processing_system(target_system_number))
            ...
            job_processing_systems[next_system].arrival(processed_job);
        }
    }

     void Job_service_complex:: termination()
    {
